...
int main(void)
{
  ...
  convert();
  while(true){
    for(right_hand = 60; right_hand >= 0; right_hand--){
      right_foot++;
      left_hand--;
      convert();
    }
    for(right_hand = 0; right_hand <= 60; right_hand++){
      right_foot--;
      left_hand++;
      convert();
    }
    for(right_hand = 60; right_hand <= 120; right_hand++){
      left_foot--;
      left_hand++;
      convert();
    }
    for(right_hand = 120; right_hand >= 60; right_hand--){
      left_foot++;
      left_hand--;
      convert();
    }
