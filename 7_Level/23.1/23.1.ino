...
int main(void)
{
  ...
  homeposition();
  convert();
  for(right_hand = 40; right_hand <= 80; right_hand++){
    left_hand--;
    convert();
  }
  for(right_hip = 90; right_hip <= 180; right_hip++){
    left_hip--;
    convert();
  }
  for(right_foot = 90; right_foot <= 140; right_foot++){
    left_foot--;
    convert();
  }
  while(true){
      for(right_hand = 80; right_hand >= 40; right_hand--){
        left_hand++;
        right_foot--;
        left_foot++;
        convert();
